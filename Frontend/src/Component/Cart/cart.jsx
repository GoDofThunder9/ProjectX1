...
const ShoppingCart = () => {
  ...
  useEffect(() => {
    const fetchCartItems = async () => {
      try {
        const response = await axios.get(`http://localhost:8080/fetchcartdetail/${storedUserId}`);
  ...
  const removeItemFromCart = async (foodId) => {
    try {
      ...
      const response = await axios.get(`http://localhost:8080/removefromcart/${foodId}/${storedUserId}`);
      if (response.status === 200) {
        // After successful removal, re-fetch the updated cart items
        const updatedResponse = await axios.get(`http://localhost:8080/fetchcartdetail/${storedUserId}`);
